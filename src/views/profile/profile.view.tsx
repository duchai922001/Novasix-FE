...
import { Card, Avatar, Button, Input, Typography, Upload, message } from "antd";
...
const Profile = () => {
  ...
  const handleSaveAvatar = async () => {
    ...
    formData.append("avatar", fileList[fileList.length - 1].originFileObj);

    try {
      // await UserService.updateInfoUser(formData);
      message.success("Cập nhật ảnh đại diện thành công!");
      asyncUserCurrent(); // Lấy thông tin mới sau khi cập nhật
    } catch (error) {
      handleError(error);
    }
  };
